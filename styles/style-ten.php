<?php
    ...
    $output .='.atmswc-style-10-pic img{
	...
	$output .='.atmswc-style-10 .atmswc-style-10-team-info .atmswc-style-10-title {
				color:#'.$title_color.';
				font-size: '.$title_fontsize.'px;
			}';	
			
	$output	.='.atmswc-style-10 .atmswc-style-10-biography{
				color: #'.$bio_text_color.';
				font-size: '.$bio_fontsize.'px;
			}';	
	$output .='.atmswc-style-10 .atmswc-style-10-designation{
				color: #'.$designation_color.';
				font-size:'.$bio_fontsize.'px;
			}';	
	$output .='.atmswc-style-10 .atmswc-style-10-mail{
				color: #'.$email_color.';
				font-size:'.$email_fontsize.'px;
			}';		
	$output .='.atmswc-style-10 .atmswc-style-10-web > a{
				color: #'.$web_color.';
				font-size:'.$web_fontsize.'px;
				box-shadow: none;
			}';
	$output .='.atmswc-style-10 .atmswc-style-10-telephone{
				color: #'.$telephone_color.';
				font-size:'.$telephone_fontsize.'px;
			}';	

	if($bio_social_icon == "round"){
		$output .='.atmswc-style-10 .atmswc-style-10-social li a{
					border-radius:50px
				}';		
	}
	...
	$output .='.atmswc-style-10 .atmswc-style-10-social li a:hover{
				color:#'.$bio_social_hcolor.';
				background: #'.$bio_social_backhcolor.';
			}';	
	$output .='.atmswc-style-10 .atmswc-style-10-header .atmswc-style-10-pic:before {
	...
	$output .='.atmswc-popup-left .atmswc-style-10-designation {
			color: #'.$designation_color.';
			font-size:'.$bio_fontsize.'px;	
			display:block;		
		}';	
	$output .='.atmswc-popup-left .atmswc-style-10-mail a{
			color: #'.$email_color.';
			font-size:'.$email_fontsize.'px;
			display:block;			
		}';	

	$output .='.atmswc-popup-left .atmswc-style-10-web > a{
			box-shadow: none;
			color: #'.$web_color.';
			font-size:'.$web_fontsize.'px;
			display:block;			
		}';	
	$output .='.atmswc-popup-left .atmswc-style-10-telephone {
			color: #'.$telephone_color.';
			font-size:'.$telephone_fontsize.'px;	
			display:block;	
		}';	
	...
	if($bio_social_icon == "round"){
		$output .='.social-popup .atmswc-style-10-social li a{
					border-radius:50px
			}';		
	}
	$output .='.social-popup .atmswc-style-10-social li a{
	...
	$output .='.social-popup .atmswc-style-10-social li a:hover{
				color:#'.$bio_social_hcolor.';
				background: #'.$bio_social_backhcolor.';
			}';	
	...
	$output.='
		<div class="atmswc-style-10">';
			$output.='
			<div class="atmswc-style-10-header">
				<div class="atmswc-style-10-pic">';
					$output .='<img src="'.$post_thumb.'" alt="" />';
				$output.='
				</div>
				<div class="atmswc-style-10-overlay">
					<a href="#" data-featherlight="#fl1'.$post->ID.'"><i class="fa fa-eye"></i></a>
				</div>
			</div>';
		     ...
		     <div class="atmswc-popup-area">
		      <div class="atmswc-popup-left">
		       ...
		       <h2>'.esc_attr(get_the_title()).'</h2>';
				...
				foreach ($sort_array as $value) {
					if($value =="designation"){
				        if($designation_sh == 1){
				        	$output .=' <span class="atmswc-style-10-designation">'.$tup_designation.'</span>';
				        }
					}
					if($value =="mail"){			
				        if($email_sh == 1){
				        	$output .=' <span class="atmswc-style-10-mail"><a href="mailto:'.$tup_email.'">'.$tup_email.'</a></span>';
				        }
					}
					if($value =="web"){			
				        if($web_sh == 1){
				        	$output .=' <span class="atmswc-style-10-web"><a href="'.esc_url($tup_web).'">'.$tup_web.'</a></span>'; 
				        }
					}
					if($value =="telephone"){	
				        if($telephone_sh == 1){
				        	$output .=' <span class="atmswc-style-10-telephone">'.$tup_telephone.'</span>'; 
				        }
					}			
				}
		       ...
		       <div class="social-popup">';
		        $output .='<ul class="atmswc-style-10-social">';
			...
			$output .='<div class="atmswc-style-10-team-info">
				<h3 class="atmswc-style-10-title">'.get_the_title().'</h3>';
			...
			foreach ($sort_array as $value) {
				...
				if($value =="mail"){			
					if($email_sh == 1){
						$output .='	<span class="atmswc-style-10-mail">'.$tup_email.'</span>';
					}
				}
				if($value =="web"){			
					if($web_sh == 1){
			...
			if($biography_sh == 1){
				$output .='<div class="atmswc-style-10-content">';
					$output .='<p class="atmswc-style-10-biography">'.$tup_biography.'</p>';
				$output.='</div>';	
			}
