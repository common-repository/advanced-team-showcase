<?php
    ...
    $output .='.atmswc-style-12-header img {
	...
	$output .='.atmswc-style-12 .atmswc-style-12-team-info .atmswc-style-12-title {
				color:#'.$title_color.';
				font-size: '.$title_fontsize.'px;
			}';		


	$output .='.atmswc-style-12 .atmswc-style-12-header:hover .atmswc-style-12-overlay{
				background: #'.$opacity_color.';
				opacity: '.$opacity.';				
			}';
	$output	.='.atmswc-style-12 .atmswc-style-12-biography{
				color: #'.$bio_text_color.';
				font-size: '.$bio_fontsize.'px;
			}';	
	$output .='.atmswc-style-12 .atmswc-style-12-pic::after{
				background: #'.$opacity_color.';
				opacity: '.$opacity.';
			}';
	$output .='.atmswc-style-12 .atmswc-style-12-designation{
				color: #'.$designation_color.';
				font-size:'.$bio_fontsize.'px;
			}';	
	$output .='.atmswc-style-12 .atmswc-style-12-mail{
				color: #'.$email_color.';
				font-size:'.$email_fontsize.'px;
			}';		
	$output .='.atmswc-style-12 .atmswc-style-12-web > a{
				box-shadow: none;
				color: #'.$web_color.';
				font-size:'.$web_fontsize.'px;
			}';
	$output .='.atmswc-style-12 .atmswc-style-12-telephone{
				color: #'.$telephone_color.';
				font-size:'.$telephone_fontsize.'px;
			}';	

	if($bio_social_icon == "round"){
		$output .='.atmswc-style-12 .atmswc-style-12-social li a{
					border-radius:50px
				}';		
	}
	...
	$output .='.atmswc-style-12 .atmswc-style-12-social li a:hover{
	...
	$output .='.atmswc-popup-left .atmswc-style-12-designation {
			color: #'.$designation_color.';
			font-size:'.$bio_fontsize.'px;	
			display:block;		
		}';	
	$output .='.atmswc-popup-left .atmswc-style-12-mail a{
			color: #'.$email_color.';
			font-size:'.$email_fontsize.'px;
			display:block;			
		}';	

	$output .='.atmswc-popup-left .atmswc-style-12-web > a{
			box-shadow: none;
			color: #'.$web_color.';
			font-size:'.$web_fontsize.'px;
			display:block;			
		}';	
	$output .='.atmswc-popup-left .atmswc-style-12-telephone {
			color: #'.$telephone_color.';
			font-size:'.$telephone_fontsize.'px;	
			display:block;	
		}';	
	...
	if($bio_social_icon == "round"){
		$output .='.social-popup .atmswc-style-12-social li a{
					border-radius:50px
			}';		
	}
	$output .='.social-popup .atmswc-style-12-social li a{
	...
	$output .='.social-popup .atmswc-style-12-social li a:hover{
				color:#'.$bio_social_hcolor.';
				background: #'.$bio_social_backhcolor.';
			}';							
	...
	$output.='
		<div class="atmswc-style-12">';

			$output.='
			<div class="atmswc-style-12-header">
			';
				$output .='<img src="'.$post_thumb.'" alt="" />';
				$output.='
				<div class="atmswc-style-12-overlay">
					<a href="#" class="atmswc-style-12-popup" data-featherlight="#fl1'.$post->ID.'"><i class="fa fa-search"></i></a>
				</div>
			</div>
			';
		     ...
		     <div class="atmswc-popup-area">
		      <div class="atmswc-popup-left">
		       ...
		       <h2>'.esc_attr(get_the_title()).'</h2>';
				foreach ($sort_array as $value) {
					if($value =="designation"){
				        if($designation_sh == 1){
				        	$output .=' <span class="atmswc-style-12-designation">'.$tup_designation.'</span>';
				        }
					}
					if($value =="mail"){			
				        if($email_sh == 1){
				        	$output .=' <span class="atmswc-style-12-mail"><a href="mailto:'.$tup_email.'">'.$tup_email.'</a></span>';
				        }
					}
					if($value =="web"){			
				        if($web_sh == 1){
				        	$output .=' <span class="atmswc-style-12-web"><a href="'.esc_url($tup_web).'">'.$tup_web.'</a></span>'; 
				        }
					}
					if($value =="telephone"){	
				        if($telephone_sh == 1){
				        	$output .=' <span class="atmswc-style-12-telephone">'.$tup_telephone.'</span>'; 
				        }
					}			
				}
		       ...
		       <div class="social-popup">';
		        $output .='<ul class="atmswc-style-12-social">';
			...
			$output .='<div class="atmswc-style-12-team-info">
				<h3 class="atmswc-style-12-title">'.get_the_title().'</h3>';
			...
			foreach ($sort_array as $value) {
				...
				if($value =="mail"){			
					if($email_sh == 1){
						$output .='	<span class="atmswc-style-12-mail">'.$tup_email.'</span>';
					}
				}
				if($value =="web"){			
					if($web_sh == 1){
			...
			if($biography_sh == 1){
				$output .='<div class="atmswc-style-12-content">';
					$output .='<p class="atmswc-style-12-biography">'.$tup_biography.'</p>';
				$output.='</div>';	
			}
