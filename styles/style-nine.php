<?php
	...
	// Applied style from meta value
	$output .='<style>';

	$output .='.atmswc-style-9{';
    ...
    $output .='.atmswc-style-9 .atmswc-style-9-pic img{
	...
	$output .='.atmswc-style-9 .atmswc-style-9-team-info .atmswc-style-9-title {
				color:#'.$title_color.';
				font-size: '.$title_fontsize.'px;
			}';	
	$output .='.atmswc-style-9 .atmswc-style-9-overlay  {
				background: #'.$opacity_color.';
				opacity: '.$opacity.';
			}';	
	$output	.='.atmswc-style-9 .atmswc-style-9-biography{
				color: #'.$bio_text_color.';
				font-size: '.$bio_fontsize.'px;
			}';

	if($bio_social_icon == "round"){
		$output .='.atmswc-style-9 .atmswc-style-9-social li a{
					border-radius:50px
				}';		
	}	
	$output .='.atmswc-style-9 .atmswc-style-9-social li a{
	...
	$output .='.atmswc-style-9 .atmswc-style-9-social li a:hover{
				color:#'.$bio_social_hcolor.';
				background: #'.$bio_social_backhcolor.';
			}';	

	$output .='.atmswc-style-9 .atmswc-style-9-designation{
				color: #'.$designation_color.';
				font-size:'.$bio_fontsize.'px;
			}';	

	$output .='.atmswc-style-9 .atmswc-style-9-mail{
			color: #'.$email_color.';
			font-size:'.$email_fontsize.'px;
		}';	
										
	$output .='.atmswc-style-9 .atmswc-style-9-web > a{
			box-shadow: none;
			color: #'.$web_color.';
			font-size:'.$web_fontsize.'px;
		}';	
	$output .='.atmswc-style-9 .atmswc-style-9-telephone{
	...
	$output .='.atmswc-popup-left .atmswc-style-9-designation {
			color: #'.$designation_color.';
			font-size:'.$bio_fontsize.'px;	
			display:block;		
		}';	
	$output .='.atmswc-popup-left .atmswc-style-9-mail a{
			color: #'.$email_color.';
			font-size:'.$email_fontsize.'px;
			display:block;			
		}';	

	$output .='.atmswc-popup-left .atmswc-style-9-web > a{
			box-shadow: none;
			color: #'.$web_color.';
			font-size:'.$web_fontsize.'px;
			display:block;			
		}';	
	$output .='.atmswc-popup-left .atmswc-style-9-telephone {
			color: #'.$telephone_color.';
			font-size:'.$telephone_fontsize.'px;	
			display:block;	
		}';	
	...
	if($bio_social_icon == "round"){
		$output .='.social-popup .atmswc-style-9-social li a{
					border-radius:50px
			}';		
	}
	$output .='.social-popup .atmswc-style-9-social li a{
	...
	$output .='.social-popup .atmswc-style-9-social li a:hover{
				color:#'.$bio_social_hcolor.';
				background: #'.$bio_social_backhcolor.';
			}';	
	...
	$output.='
		<div class="atmswc-style-9">';
			$output.='
			<div class="atmswc-style-9-header">
				<div class="atmswc-style-9-pic">';
					$output .='<img src="'.$post_thumb.'" alt="" />';
				$output.='
				</div>
				<div class="atmswc-style-9-overlay">';
					if($biography_sh == 1){
						$output .='<div class="atmswc-style-9-content">';
							$output .='<p class="atmswc-style-9-biography">'.$tup_biography.'</p>';
						$output.='</div>';
					}
					$output .='
					<ul class="atmswc-style-9-popup">
						<li><a href="#" data-featherlight="#fl1'.$post->ID.'"><i class="fa fa-eye"></i></a></li>
					</ul>
				</div>
			</div>';
		     ...
		     <div class="atmswc-popup-area">
		      <div class="atmswc-popup-left">
		       ...
		       <h2>'.esc_attr(get_the_title()).'</h2>';
				foreach ($sort_array as $value) {
					if($value =="designation"){
				        if($designation_sh == 1){
				        	$output .=' <span class="atmswc-style-9-designation">'.$tup_designation.'</span>';
				        }
					}
					if($value =="mail"){			
				        if($email_sh == 1){
				        	$output .=' <span class="atmswc-style-9-mail"><a href="mailto:'.$tup_email.'">'.$tup_email.'</a></span>';
				        }
					}
					if($value =="web"){			
				        if($web_sh == 1){
				        	$output .=' <span class="atmswc-style-9-web"><a href="'.esc_url($tup_web).'">'.$tup_web.'</a></span>'; 
				        }
					}
					if($value =="telephone"){	
				        if($telephone_sh == 1){
				        	$output .=' <span class="atmswc-style-9-telephone">'.$tup_telephone.'</span>'; 
				        }
					}			
				}
		       ...
		       <div class="social-popup">';
		        $output .='<ul class="atmswc-style-9-social">';
			...
			$output .='<div class="atmswc-style-9-team-info">
				<h3 class="atmswc-style-9-title">'.get_the_title().'</h3>';
			foreach ($sort_array as $value) {
				if($value =="designation"){
					if($designation_sh == 1){
						$output .='	<span class="atmswc-style-9-designation">'.$tup_designation.'</span>';
				...
				if($value =="mail"){			
					if($email_sh == 1){
						$output .='	<span class="atmswc-style-9-mail">'.$tup_email.'</span>';
					}
				}
				if($value =="web"){			
					if($web_sh == 1){
