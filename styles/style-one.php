<?php
	...
	$output .='.atmswc-style-1-title{
				color:#'.$title_color.';
				font-size: '.$title_fontsize.'px;
			}';	
	$output .='.atmswc-style-1 .atmswc-style-1-pic:after {
				background: #'.$opacity_color.';
				opacity: '.$opacity.';
			}';	
	$output	.='.atmswc-style-1 .atmswc-style-1-biography{
				color: #'.$bio_text_color.';
				font-size: '.$bio_fontsize.'px;
			}';

	if($bio_social_icon == "round"){
		$output .='.atmswc-style-1 .atmswc-style-1-social li a{
					border-radius:50px
				}';		
	}	
	$output .='.atmswc-style-1 .atmswc-style-1-social li a{
	...
	$output .='.atmswc-style-1 .atmswc-style-1-social li a:hover{
				color:#'.$bio_social_hcolor.';
				background: #'.$bio_social_backhcolor.';
			}';	

	$output .='.atmswc-style-1 .atmswc-style-1-designation{
				color: #'.$designation_color.';
				font-size:'.$bio_fontsize.'px;
			}';	

	$output .='.atmswc-style-1 .atmswc-style-1-mail{
			color: #'.$email_color.';
			font-size:'.$email_fontsize.'px;
		}';	
										
	$output .='.atmswc-style-1 .atmswc-style-1-web > a{
			box-shadow: none;
			color: #'.$web_color.';
			font-size:'.$web_fontsize.'px;
		}';	
	$output .='.atmswc-style-1 .atmswc-style-1-telephone{
	...
	$output .='.atmswc-popup-left .atmswc-style-1-designation {
			color: #'.$designation_color.';
			font-size:'.$bio_fontsize.'px;	
			display:block;		
		}';	
	$output .='.atmswc-popup-left .atmswc-style-1-mail a{
			color: #'.$email_color.';
			font-size:'.$email_fontsize.'px;
			display:block;			
		}';	

	$output .='.atmswc-popup-left .atmswc-style-1-web > a{
			box-shadow: none;
			color: #'.$web_color.';
			font-size:'.$web_fontsize.'px;
			display:block;			
		}';	
	$output .='.atmswc-popup-left .atmswc-style-1-telephone {
			color: #'.$telephone_color.';
			font-size:'.$telephone_fontsize.'px;	
			display:block;	
		}';	
	...
	if($bio_social_icon == "round"){
		$output .='.social-popup .atmswc-style-1-social li a{
					border-radius:50px
			}';		
	}
	$output .='.social-popup .atmswc-style-1-social li a{
	...
	$output .='.social-popup .atmswc-style-1-social li a:hover{
				color:#'.$bio_social_hcolor.';
				background: #'.$bio_social_backhcolor.';
			}';									
	$output .='</style>'; 
	...
	$output .='<div class="postgrids-col-lg-3 postgrids-col-md-4 postgrids-col-sm-2 postgrids-col-xs-1">';
	 	$output .='<div class="atmswc-style-1">';
			$output .='<div class="atmswc-style-1-pic">';
				$output .='<img src="'.$post_thumb.'" alt="" />';
			$output .='</div>';
			$output .='<div class="atmswc-style-1-content">';
			if($biography_sh == 1){

				$content = apply_filters( 'the_content', get_the_content() );
		    	$output .='<p class="atmswc-style-1-biography">'.$tup_biography.'<span><a href="#" data-featherlight="#fl1'.$post->ID.'"><i class="fa fa-eye"></i></a></span></p>';
			     ...
			     <div class="atmswc-popup-area">
			      <div class="atmswc-popup-left">
			       ...
			       <h2>'.esc_attr(get_the_title()).'</h2>';
					...
					foreach ($sort_array as $value) {  // show details with sorting
						if($value =="designation"){
					        if($designation_sh == 1){
					        	$output .=' <span class="atmswc-style-1-designation">'.$tup_designation.'</span>';
					        }
						}
						if($value =="mail"){			
					        if($email_sh == 1){
					        	$output .=' <span class="atmswc-style-1-mail"><a href="mailto:'.$tup_email.'">'.$tup_email.'</a></span>';
					        }
						}
						if($value =="web"){			
					        if($web_sh == 1){
					        	$output .=' <span class="atmswc-style-1-web"><a href="'.esc_url($tup_web).'">'.$tup_web.'</a></span>'; 
					        }
						}
						if($value =="telephone"){	
					        if($telephone_sh == 1){
					        	$output .=' <span class="atmswc-style-1-telephone">'.$tup_telephone.'</span>'; 
					        }
						}			
					}
			       ...
			       <div class="social-popup">';
			        $output .='<ul class="atmswc-style-1-social">';
					foreach ($sort_social as $s_s) {	
						if($s_s =="fb"){							        
			...
			$output .='</div>';
			$output .='<div class="atmswc-style-1-team-info">
				<h3 class="atmswc-style-1-title">'.get_the_title().'</h3>';
			...
			foreach ($sort_array as $value) {  // show details with sorting
				...
				if($value =="mail"){			
					if($email_sh == 1){
						$output .='	<span class="atmswc-style-1-mail">'.$tup_email.'</span>';
					}
				}
				if($value =="web"){			
					if($web_sh == 1){
