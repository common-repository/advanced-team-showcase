<?php
    ...
    $output .='.atmswc-style-6-pic a img{
	...
	$output .='.atmswc-style-6 .atmswc-style-6-team-info .atmswc-style-6-title{
			color:#'.$title_color.';
			font-size: '.$title_fontsize.'px;
		}';	

	$output	.='.atmswc-style-6 .atmswc-style-6-biography{
			color: #'.$bio_text_color.';
			font-size: '.$bio_fontsize.'px;
		}';	
	$output .='.atmswc-style-6 .atmswc-style-6-designation{
			color: #'.$designation_color.';
			font-size:'.$bio_fontsize.'px;
		}';	
	$output .='.atmswc-style-6 .atmswc-style-6-mail{
			color: #'.$email_color.';
			font-size:'.$email_fontsize.'px;
		}';		
	$output .='.atmswc-style-6 .atmswc-style-6-web > a{
			box-shadow: none;
			color: #'.$web_color.';
			font-size:'.$web_fontsize.'px;
		}';
	$output .='.atmswc-style-6 .atmswc-style-6-telephone{
			color: #'.$telephone_color.';
			font-size:'.$telephone_fontsize.'px;
		}';	

	if($bio_social_icon == "round"){
		$output .='.atmswc-style-6 .atmswc-style-6-social li a{
					border-radius:50px
				}';		
	}
	...
	$output .='.atmswc-style-6 .atmswc-style-6-social li a:hover{
	...
	$output .='.atmswc-popup-left .atmswc-style-6-designation {
			color: #'.$designation_color.';
			font-size:'.$bio_fontsize.'px;	
			display:block;		
		}';	
	$output .='.atmswc-popup-left .atmswc-style-6-mail a{
			color: #'.$email_color.';
			font-size:'.$email_fontsize.'px;
			display:block;			
		}';	

	$output .='.atmswc-popup-left .atmswc-style-6-web > a{
			box-shadow: none;
			color: #'.$web_color.';
			font-size:'.$web_fontsize.'px;
			display:block;			
		}';	
	$output .='.atmswc-popup-left .atmswc-style-6-telephone {
			color: #'.$telephone_color.';
			font-size:'.$telephone_fontsize.'px;	
			display:block;	
		}';	
	...
	if($bio_social_icon == "round"){
		$output .='.social-popup .atmswc-style-6-social li a{
					border-radius:50px
			}';		
	}
	$output .='.social-popup .atmswc-style-6-social li a{
	...
	$output .='.social-popup .atmswc-style-6-social li a:hover{
				color:#'.$bio_social_hcolor.';
				background: #'.$bio_social_backhcolor.';
			}';																				
	$output .='</style>'; 
	...
	$output .='<div class="postgrids-col-lg-3 postgrids-col-md-4 postgrids-col-sm-2 postgrids-col-xs-1">';
	 	$output .='<div class="atmswc-style-6">';
		$output.='
		<div class="atmswc-style-6-header">';
			$output .='<div class="atmswc-style-6-pic">';
		     ...
		     <div class="atmswc-popup-area">
		      <div class="atmswc-popup-left">
		       ...
		       <h2>'.esc_attr(get_the_title()).'</h2>';
				...
				foreach ($sort_array as $value) {
					if($value =="designation"){
				        if($designation_sh == 1){
				        	$output .=' <span class="atmswc-style-6-designation">'.$tup_designation.'</span>';
				        }
					}
					if($value =="mail"){			
				        if($email_sh == 1){
				        	$output .=' <span class="atmswc-style-6-mail"><a href="mailto:'.$tup_email.'">'.$tup_email.'</a></span>';
				        }
					}
					if($value =="web"){			
				        if($web_sh == 1){
				        	$output .=' <span class="atmswc-style-6-web"><a href="'.esc_url($tup_web).'">'.$tup_web.'</a></span>'; 
				        }
					}
					if($value =="telephone"){	
				        if($telephone_sh == 1){
				        	$output .=' <span class="atmswc-style-6-telephone">'.$tup_telephone.'</span>'; 
				        }
					}			
				}
		       ...
		       <div class="social-popup">';
		        $output .='<ul class="atmswc-style-6-social">';
			...
			$output .='<div class="atmswc-style-6-team-info">
				<h3 class="atmswc-style-6-title">'.get_the_title().'</h3>';
				foreach ($sort_array as $value) {
					if($value =="designation"){
						if($designation_sh == 1){
							$output .='	<span class="atmswc-style-6-designation">'.$tup_designation.'</span>';
					...
					if($value =="mail"){			
						if($email_sh == 1){
							$output .='	<span class="atmswc-style-6-mail">'.$tup_email.'</span>';
						}
					}
					if($value =="web"){			
						if($web_sh == 1){
			...
			if($biography_sh == 1){
				$output .='<div class="atmswc-style-6-content">';
					$output .='<p class="atmswc-style-6-biography">'.$tup_biography.'</p>';
				$output.='</div>';	
			}
